	...
	public class WebSocketMiddleware : IMiddleware
	{
		...
		public async Task InvokeAsync(HttpContext context, RequestDelegate next)
		{
			...
			else
			{
				...
				string endPoint = $"{webSocketProtocol}{context.Request.Host}{context.Request.Path.Value}";
				...
				if (sub != null)
				{
					...
					while (ws.State == WebSocketState.Open && !context.RequestAborted.IsCancellationRequested)
					{
						...
						else
						{
							...
							{
								this.logger.LogError($"Received empty event notification message body:\r\n{socketData}");
							}
						}
					}
				...
				{
					this.logger.LogWarning($"Rejected websocket connection requested from {sub.Channel.Endpoint}; topic:{sub.Topic}. Subscription not found for this endpoint.");
				}
			}
		}
	}
